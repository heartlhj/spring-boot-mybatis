...
//		return gogoing.DateToString(this, true);
//	};
//	
		...
		DateToString : function(d, useTime) {
			if (d) {
				return d.getFullYear()
						+ "-"
						+ gogoing.StrZeroAdd(d.getMonth() + 1)
						+ "-"
						+ gogoing.StrZeroAdd(d.getDate())
						+ ((useTime) ? " " + gogoing.StrZeroAdd(d.getHours()) + ":"
								+ gogoing.StrZeroAdd(d.getMinutes()) + ":"
								+ gogoing.StrZeroAdd(d.getSeconds()) : "");
			} else {
				return "";
			}
		},
...
//			$('#'+obj.eleId).remove();
		...
		openModalWin : function(name) {
			...
			var submitFn = gogoing.doSomeThing;//模态窗口提交后回调方法
		...
		doSomeThing :function(param){
			//alert('回调方法');
			//alert("从模态窗口返回的参数："+gogoing.ToJson(param));
			var name = param.name;
		...
		TableDictTrans:function(tableCode,colCode,colValue){
			if(_TableDictData[tableCode]==null){
				...
				var retList = gogoing.InvokeMethod(url,obj);
				if(retList!=null){
		...
		TableDictGetValueList : function(tableCode, colCode){
			...
			if(gogoing.IsNull(colCode)){
				var tableObj={};
				for(key in obj){
					tableObj[key]=gogoing.ResolveToList(obj,key);
				}
				return tableObj;
			}

			
			return gogoing.ResolveToList(obj,colCode);
		},
		...
		GetGidValue : function(gidCode){
			if(gogoing.IsNull(gidCode)){
				gogoing.Alert("gidCode不能为空");
				return ;
			}
			...
			var gidValue = null;
			var obj = gogoing.InvokeMethod(url,paramObj);
			if(!gogoing.IsNull(obj)){
		...
		GetNow : function(){
			...
			if(!gogoing.IsNull(obj)){
				...
				gogoing.Alert("获取服务器当前时间失败，转用当前系统时间！");
				date = new Date();
			}
			return date;
		},
		...
		Print : function(Vflag){
			var flag = false;
			if(gogoing.IsNull(Vflag) || 'true' == Vflag){
				flag = true;
			}
			if(!flag){
...
//			if(!gogoing.IsNull(privates) && privates.indexOf(privCode) > -1){
//				flag = true;
//				return flag;
//			}else{
//				flag = false;
...
//			}
		...
		GenButton : function (buttonGroup){
			...
			var isAdmin = gogoing.IsSuperadminRole();
			if(!isAdmin){
				for(var i=0 ; i< buttons.length; i++){
					if(gogoing.HasPriv(buttons[i]) ){
		...
		WaitDivHidden : function(){
			var loadingModalObj = gogoing.GetObj("includes-index_backdrop-loadingModal");
